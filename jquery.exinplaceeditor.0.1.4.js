...
(function($){
	...
	$.ex.inPlaceEditor = function(idx , targets , option){
		...
		if (/INPUT|TEXTAREA/.test( c.tag )) {
			...
			c.editor = c.target;
			var labelTag = c.convertCR == 'li' ? 'ul' : c.directEdit ? 'a' : 'span';
			c.label = $('<' + labelTag + '/>');
			if (labelTag == 'a') {
				c.label.attr('href','#');
			}
			c.editor.before(c.label);
			c.editor.val( o.getEditorValue(c.editor.val()) );
			...
			c.editor.val( o.getEditorValue(c.label.html()) );
			if (c.label.find('*[tagName!=' + c.convertCR + ']').size()) {
				c.htmlEditor = true;
			}
			c.label.html( o.getConvertValue() );
	...
	$.extend($.ex.inPlaceEditor.prototype, {
		...
		_convertElementToCR : function( text ){
			var o = this , c = o.config;
			if (text.length && c.isTextarea && !(c.htmlEditor && !c.htmlEditorAutoConvertCR)) {
				if (c.convertCR == 'br'){
		...
		_convertCRToElement : function( text ){
			var o = this , c = o.config;
			if (text.length && c.isTextarea && !(c.htmlEditor && !c.htmlEditorAutoConvertCR)) {
				if( c.convertCR == 'br'){
					text = text.replace(/\n/g,'<br/>');
		...
		getConvertValue : function(){
			...
			if (c.htmlEditorAutoConvertCR) {
				val = o._convertCRToElement(val);
			}
			return val == '' ? c.nulltext : val;
		},
		getEditorValue : function(val){
			var o = this , c = o.config;
			var val = arguments.length ? val : o.getValue();
			val = o._convertElementToCR(val);
			return val == c.nulltext ? '' : val;
		},
