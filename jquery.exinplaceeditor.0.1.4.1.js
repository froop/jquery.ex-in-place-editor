...
(function($){
	...
	$.ex.inPlaceEditor = function(idx , targets , option){
		...
		if (/INPUT|TEXTAREA/.test( c.tag )) {
			...
			c.label.after( c.editor.hide() );
			c.editor.show().val( o._HTMLtoINPUT(c.label.html()) );
			c.label.html( o._INPUTtoHTML(c.editor.val()) );
	...
	$.extend($.ex.inPlaceEditor.prototype, {
		...
		_INPUTtoHTML : function( input , noNullReplace){
			...
			return !noNullReplace ? input == '' ? c.nulltext : input : input;
		},
		_HTMLtoINPUT : function( html , noNullReplace){
			...
			if (!c.htmlEditor) {
				...
				if( c.convertCR == 'li'){
					html = html.replace(/\s*<LI>/ig,'').replace(/\<\/LI>\s*/ig,'%%%CR%%%').replace(/%%%CR%%%$/ig,'');
				}
